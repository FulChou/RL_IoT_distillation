...
from tianshou.trainer import offpolicy_trainer_key
from tianshou.data import Collector, VectorReplayBuffer
from utils import CountTime
...
def test_dqn(args=get_args()):
    ...
    log_file = f'seed_{args.seed}_{t0}-{args.task.replace("-", "_")}' + '-random-500-0.95-200v3'
    log_path = os.path.join(args.logdir, args.task, 'dqn', log_file)
    print(log_path)
    writer = SummaryWriter(log_path)
    writer.add_text("args", str(args))
    logger = BasicLogger(writer)
    # matlab_lib = key_state_by.initialize()
    def save_fn(policy):
    ...
    def update_student(best_teacher_policy=None, sample_size=1, logger=None, step=0,):
        if len(train_collector.buffer) >= 1e3:
            ...
            t_idxs = sample([i for i in range(len(indice_all))], 950//2)
            t_idxs.sort()
            for idx in t_idxs:
                if idx < 1e3-1:
                    idxs.append(idx)
                    idxs.append(idx+1)
            indice_random = indice_all[idxs]
            batch_random = batch_all[idxs]
            buffer_id = [int(i // 1e4) for i in indice_random]
            key_buffer.add(batch_random, buffer_id)
    
            train_collector.buffer.reset()


    # test train_collector and start filling replay buffer
    train_collector.collect(n_step=100 * args.training_num)
    # trainer
    result = offpolicy_trainer_key(
        policy, train_collector, test_collector, args.epoch,
        args.step_per_epoch, args.step_per_collect, args.test_num,
        args.batch_size, train_fn=train_fn, test_fn=test_fn,
        stop_fn=stop_fn, save_fn=save_fn, logger=logger, update_student_fn=update_student, key_buffer=key_buffer,

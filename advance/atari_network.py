...
class student_DQN(DQN):
    ...
    def __init__(
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(16, 16, kernel_size=4, stride=2), nn.ReLU(inplace=True),
            nn.Conv2d(16, 16, kernel_size=3, stride=1), nn.ReLU(inplace=True),
            nn.Flatten())
        with torch.no_grad():
            self.output_dim = np.prod(
                self.net(torch.zeros(1, c, h, w)).shape[1:])

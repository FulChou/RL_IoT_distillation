...
def test_dqn(args=get_args()):
    ...
    log_file = f'seed_{args.seed}_{t0}-{args.task.replace("-", "_")}'+'assig'
    ...
    def update_student(best_teacher_policy=None, sample_size=1, logger=None, step=0, is_update_student=True):
        teacher_params = policy.model.state_dict()  # 直接复制的方法改成蒸馏：
        policy_student.model.load_state_dict(teacher_params)

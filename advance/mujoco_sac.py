...
def get_args():
    ...
    parser.add_argument('--hidden-student-sizes', type=int, nargs='*', default=[64, 64])
...
def test_sac(args=get_args()):
    ...
    # student model:
    net_a_student = Net(args.state_shape, hidden_sizes=args.hidden_student_sizes, device=args.device) # 256 -> 32
    ...
    test_collector = Collector(policy, test_envs)  # test_policy
    test_student_collector = Collector(policy_student, test_envs)  # test student policy

    train_collector.collect(n_step=args.start_timesteps, random=True)
    # log
    t0 = datetime.datetime.now().strftime("%m%d_%H%M%S")
    log_file = f'seed_{args.seed}_{t0}-{args.task.replace("-", "_")}_always_64_clamp_400'
    log_path = os.path.join(args.logdir, args.task, 'sac', log_file)
    writer = SummaryWriter(log_path)
    writer.add_text("args", str(args))
    logger = BasicLogger(writer)
    print(log_path, 'have logger!!!', args.device)
    ...
    def save_student_policy_fn(policy_student):
        torch.save(policy_student.state_dict(), os.path.join(log_path, 'policy_student.pth'))
    ...
    if not args.watch:
        ...
        result = offpolicy_trainer(
            ...
            args.batch_size, save_fn=save_fn, save_student_policy_fn=save_student_policy_fn, logger=logger,

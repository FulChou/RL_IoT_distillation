...
print('info device:', device, 'env reward:', env.spec.reward_threshold,'1000epoch')
...
train_collector = ts.data.Collector(student_policy, train_envs, ts.data.VectorReplayBuffer(50000, 10), exploration_noise=True)
...
def update_student():
    ...
    if len(train_collector.buffer) > sample_size:
        ...
        std = torch.tensor([1e-6] * len(teacher.logits[0]), device=device,dtype=torch.float)
        stds = torch.stack([std for _ in range(len(teacher.logits))])
...
result = ts.trainer.offpolicy_trainer(
    ...
    max_epoch=1000, step_per_epoch=10000, step_per_collect=128,

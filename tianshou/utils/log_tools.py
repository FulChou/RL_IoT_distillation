...
class BaseLogger(ABC):
    ...
    def log_kl_loss(self, update_result: dict, step: int) -> None:
        """Use writer to log statistics generated during updating.
    ...
    def log_test_student_data(self, collect_result: dict, step: int) -> None:
        """Use writer to log statistics generated during evaluating.
...
class BasicLogger(BaseLogger):
    ...
    def __init__(
        ...
        self.last_log_test_student_step = -1
        self.last_log_update_step = -1
        self.last_log_kl_loss = -1
    ...
    def log_test_student_data(self, collect_result: dict, step: int) -> None:
        ...
        rew, rew_std, len_, len_std = rews.mean(), rews.std(), lens.mean(), lens.std()
        collect_result.update(rew=rew, rew_std=rew_std, len=len_, len_std=len_std)
        if step - self.last_log_test_student_step >= self.test_interval:
            self.write("test_student/rew", step, rew)
            self.write("test_student/len", step, len_)
            self.write("test_student/rew_std", step, rew_std)
            self.write("test_student/len_std", step, len_std)
            self.last_log_test_student_step = step
    ...
    def log_kl_loss(self, update_result: dict, step: int) -> None:
        ...
        :param int step: stands for the timestep the collect_result being logged.
        """
        if step - self.last_log_kl_loss >= self.update_interval:
            for k, v in update_result.items():
                self.write(k, step, v)
            self.last_log_kl_loss = step
